...
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    a^2 - a + 3 = a^2 - 6 * a + 9 + 5 * a - 6 := by ring
...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z^4 - z^3 - z^2 + 2 * z + 1 = z^2 * (z^2 - 1) - z * (z^2 - 2) + 1 := by ring
    _ = z^2 * (z^2 - 1) - z * 0 + 1 := by rw [h1]
    _ = z^2 * (z^2 - 1) + 1 := by ring
    _ = z^2 * (z^2 - 2) + (z^2 - 2) + 3 := by ring
    _ = z^2 * 0 + 0 + 3 := by rw [h1]
    _ = 3 := by ring
...
example {a b : ℝ} (h1 : a + 2 * b = 4) (h2 : a - b = 1) : a = 2 :=
  calc
    a = 2 / 3 * ((a - b) + (a + 2 * b) / 2) := by ring
    _ = 2 / 3 * (1 + 4 / 2) := by rw [h1, h2]
    _ = 2 := by ring

example {u v : ℝ} (h1 : u + 1 = v) : u ^ 2 + 3 * u + 1 = v ^ 2 + v - 1 :=
  calc
    u^2 + 3 * u + 1 = u^2 + 3 * u + 9 / 4 - 5 / 4 := by ring
    _ = (u + 3/2)^2 - 5/4 := by ring
    _ = ((u + 1) + 1/2)^2 - 5/4 := by ring
    _ = (v + 1/2)^2 - 5/4 := by rw [h1]
    _ = v^2 + v + 1/4 - 5/4 := by ring
    _ = v^2 + v - 1 := by ring

example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
    t ^ 4 + 3 * t ^ 3 - 3 * t ^ 2 - 2 * t - 2 = 10 * t + 2 :=
  calc
    t ^ 4 + 3 * t ^ 3 - 3 * t ^ 2 - 2 * t - 2 = t^2 * (t^2 - 3) + t * (3 * t^2 - 2) - 2 := by ring
    _ = t^2 * ((t^2 - 4) + 1) + 3 * t * ((t^2 - 4) + 10 / 3) - 2 := by ring
    _ = t^2 * (0 + 1) + 3 * t * (0 + 10 / 3) -  2 := by rw [ht]
    _ = t^2 * 1 + 3 * t * 10 / 3 - 2 := by ring
    _ = t^2 + 10 * t - 2 := by ring
